...
tap_1.default.test('UNBOUNDED_SEMITYPED_LENGTH_PREFIX: [] ([])', function (test) {
    var context = encoder_1.getDefaultEncodingContext();
    var value = [];
    ...
    var bytesWritten = encode_1.UNBOUNDED_SEMITYPED_LENGTH_PREFIX(buffer, 0, value, options, context);
    var result = decode_1.UNBOUNDED_SEMITYPED_LENGTH_PREFIX(buffer, 0, options);
    test.is(bytesWritten, 1);
    test.is(bytesWritten, result.bytes);
    test.strictSame(result.value, value);
    test.end();
});
tap_1.default.test('UNBOUNDED_SEMITYPED_LENGTH_PREFIX (scalars)', function (test) {
    fc.assert(fc.property(fc.array(fc.oneof(fc.constant(null), fc.boolean(), fc.integer(), fc.float(), fc.double(), fc.string({
        ...
        var offset = 0;
        var bytesWritten = encode_1.UNBOUNDED_SEMITYPED_LENGTH_PREFIX(buffer, offset, value, {
            prefixEncodings: []
        }, context);
        var result = decode_1.UNBOUNDED_SEMITYPED_LENGTH_PREFIX(buffer, offset, {
            prefixEncodings: []
        });
...
tap_1.default.test('UNBOUNDED_TYPED_LENGTH_PREFIX ([], integer)', function (test) {
    fc.assert(fc.property(fc.array(fc.integer()), function (value) {
        ...
        var bytesWritten = encode_1.UNBOUNDED_TYPED_LENGTH_PREFIX(buffer, offset, value, {
            prefixEncodings: [],
            encoding: encoding
        }, context);
        var result = decode_1.UNBOUNDED_TYPED_LENGTH_PREFIX(buffer, offset, {
            prefixEncodings: [],
            encoding: encoding
        });
        return bytesWritten > 0 && result.bytes === bytesWritten &&
