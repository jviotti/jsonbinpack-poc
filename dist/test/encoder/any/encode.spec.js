...
tap_1.default.test('ANY__TYPE_PREFIX: should encode null as 0x26', function (test) {
    ...
    test.strictSame(buffer.getBuffer(), Buffer.from([0x26]));
...
tap_1.default.test('ANY__TYPE_PREFIX: should encode true as 0x16', function (test) {
    var context = encoder_1.getDefaultEncodingContext();
    var buffer = new encoder_1.ResizableBuffer(Buffer.allocUnsafe(1));
...
tap_1.default.test('ANY__TYPE_PREFIX: should encode 3.14 as 0x56 + double', function (test) {
    var context = encoder_1.getDefaultEncodingContext();
    var buffer = new encoder_1.ResizableBuffer(Buffer.allocUnsafe(4));
...
tap_1.default.test('ANY__TYPE_PREFIX: should encode 256 as 0x36 + varint', function (test) {
    var context = encoder_1.getDefaultEncodingContext();
    ...
    test.strictSame(buffer.getBuffer(), Buffer.from([0x36, 0x80, 0x02]));
...
tap_1.default.test('ANY__TYPE_PREFIX: should encode 0 as 0x14', function (test) {
    var context = encoder_1.getDefaultEncodingContext();
    var buffer = new encoder_1.ResizableBuffer(Buffer.allocUnsafe(1));
    var bytesWritten = encode_1.ANY__TYPE_PREFIX(buffer, 0, 0, {}, context);
    test.strictSame(buffer.getBuffer(), Buffer.from([0x14]));
    test.is(bytesWritten, 1);
    test.end();
});
tap_1.default.test('ANY__TYPE_PREFIX: should encode -257 as 0x46 + 256 varint', function (test) {
    var context = encoder_1.getDefaultEncodingContext();
    var buffer = new encoder_1.ResizableBuffer(Buffer.allocUnsafe(3));
    var bytesWritten = encode_1.ANY__TYPE_PREFIX(buffer, 0, -257, {}, context);
    test.strictSame(buffer.getBuffer(), Buffer.from([0x46, 0x80, 0x02]));
    test.is(bytesWritten, 3);
    test.end();
});
tap_1.default.test('ANY__TYPE_PREFIX: should encode -1 as 0x15', function (test) {
    var context = encoder_1.getDefaultEncodingContext();
    var buffer = new encoder_1.ResizableBuffer(Buffer.allocUnsafe(1));
