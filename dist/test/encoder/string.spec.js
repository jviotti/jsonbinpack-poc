...
tap_1.default.test('BOUNDED_PREFIX_LENGTH_8BIT_FIXED (ASCII)', function (test) {
    ...
    fc.assert(fc.property(arbitrary, function (_a) {
        ...
        var bytesWritten = encode_1.BOUNDED_PREFIX_LENGTH_8BIT_FIXED(buffer, offset, value, {
            minimum: minimum, maximum: maximum
        }, context);
        var result = decode_1.BOUNDED_PREFIX_LENGTH_8BIT_FIXED(buffer, offset, {
            minimum: minimum, maximum: maximum
        });
...
tap_1.default.test('ROOF_PREFIX_LENGTH_ENUM_VARINT (ASCII)', function (test) {
    ...
    fc.assert(fc.property(arbitrary, function (_a) {
        ...
        var bytesWritten = encode_1.ROOF_PREFIX_LENGTH_ENUM_VARINT(buffer, offset, value, {
            maximum: maximum
        }, context);
        var result = decode_1.ROOF_PREFIX_LENGTH_ENUM_VARINT(buffer, offset, {
            maximum: maximum
        });
        return bytesWritten > 0 && result.bytes === bytesWritten && result.value === value;
    }), {
...
tap_1.default.test('FLOOR_PREFIX_LENGTH_ENUM_VARINT (ASCII)', function (test) {
    ...
    fc.assert(fc.property(arbitrary, function (_a) {
        ...
        var context = encoder_1.getDefaultEncodingContext();
        var buffer = new encoder_1.ResizableBuffer(Buffer.allocUnsafe(2048));
        var bytesWritten = encode_1.FLOOR_PREFIX_LENGTH_ENUM_VARINT(buffer, offset, value, {
            minimum: minimum
        }, context);
        var result = decode_1.FLOOR_PREFIX_LENGTH_ENUM_VARINT(buffer, offset, {
            minimum: minimum
        });
        return bytesWritten > 0 && result.bytes === bytesWritten && result.value === value;
    }), {
...
tap_1.default.test('BOUNDED_PREFIX_LENGTH_8BIT_FIXED: shared string', function (test) {
    var context = encoder_1.getDefaultEncodingContext();
    ...
    var decode1 = decode_1.BOUNDED_PREFIX_LENGTH_8BIT_FIXED(buffer, 0, options);
...
tap_1.default.test('ROOF_PREFIX_LENGTH_ENUM_VARINT: shared string', function (test) {
    var context = encoder_1.getDefaultEncodingContext();
    var buffer = new encoder_1.ResizableBuffer(Buffer.allocUnsafe(7));
    var options = {
        maximum: 4
    ...
    test.is(bytesWritten1, 4);
    test.is(bytesWritten2, 3);
    var decode1 = decode_1.ROOF_PREFIX_LENGTH_ENUM_VARINT(buffer, 0, options);
    test.is(decode1.bytes, bytesWritten1);
    test.is(decode1.value, 'foo');
    var decode2 = decode_1.ROOF_PREFIX_LENGTH_ENUM_VARINT(buffer, decode1.bytes, options);
    test.is(decode2.bytes, bytesWritten2);
    test.is(decode2.value, 'foo');
    test.end();
});
tap_1.default.test('FLOOR_PREFIX_LENGTH_ENUM_VARINT: shared string', function (test) {
    var context = encoder_1.getDefaultEncodingContext();
    var buffer = new encoder_1.ResizableBuffer(Buffer.allocUnsafe(7));
    var options = {
        minimum: 2
    ...
    test.is(bytesWritten1, 4);
    test.is(bytesWritten2, 3);
    var decode1 = decode_1.FLOOR_PREFIX_LENGTH_ENUM_VARINT(buffer, 0, options);
    test.is(decode1.bytes, bytesWritten1);
    test.is(decode1.value, 'foo');
    var decode2 = decode_1.FLOOR_PREFIX_LENGTH_ENUM_VARINT(buffer, decode1.bytes, options);
    test.is(decode2.bytes, bytesWritten2);
    test.is(decode2.value, 'foo');
    test.end();
});
