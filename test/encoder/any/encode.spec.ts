...
tap.test('ANY__TYPE_PREFIX: should encode null as 0x17', (test) => {
...
tap.test('ANY__TYPE_PREFIX: should encode false as 0x07', (test) => {
  const context: EncodingContext = getDefaultEncodingContext()
  const buffer: ResizableBuffer = new ResizableBuffer(Buffer.allocUnsafe(1))
...
tap.test('ANY__TYPE_PREFIX: should encode true as 0x0f', (test) => {
  const context: EncodingContext = getDefaultEncodingContext()
  const buffer: ResizableBuffer = new ResizableBuffer(Buffer.allocUnsafe(1))
...
tap.test('ANY__TYPE_PREFIX: should encode 256 as 0x1f + varint', (test) => {
  const context: EncodingContext = getDefaultEncodingContext()
...
tap.test('ANY__TYPE_PREFIX: should encode 255 as 0x05 0xff', (test) => {
  const context: EncodingContext = getDefaultEncodingContext()
  const buffer: ResizableBuffer = new ResizableBuffer(Buffer.allocUnsafe(2))
...
tap.test('ANY__TYPE_PREFIX: should encode 0 as 0x0d', (test) => {
  const context: EncodingContext = getDefaultEncodingContext()
  const buffer: ResizableBuffer = new ResizableBuffer(Buffer.allocUnsafe(1))
...
tap.test('ANY__TYPE_PREFIX: should encode -257 as 0x27 + 256 varint', (test) => {
  const context: EncodingContext = getDefaultEncodingContext()
  const buffer: ResizableBuffer = new ResizableBuffer(Buffer.allocUnsafe(3))
...
tap.test('ANY__TYPE_PREFIX: should encode -1 as 0x0e', (test) => {
  const context: EncodingContext = getDefaultEncodingContext()
  const buffer: ResizableBuffer = new ResizableBuffer(Buffer.allocUnsafe(1))
...
tap.test('ANY__TYPE_PREFIX: should encode -256 as 0x06 0xff', (test) => {
  const context: EncodingContext = getDefaultEncodingContext()
  const buffer: ResizableBuffer = new ResizableBuffer(Buffer.allocUnsafe(2))
  const bytesWritten: number = ANY__TYPE_PREFIX(buffer, 0, -256, {}, context)
...
tap.test('ANY__TYPE_PREFIX: should encode {foo:"bar",baz:1}', (test) => {
  ...
  test.strictSame(buffer.getBuffer(), Buffer.from([
    ...
    0x15 // Value 1 + 1 with positive integer type tag
  ]))
